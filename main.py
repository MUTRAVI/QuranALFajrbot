...
# قائمة المدن الأساسية لضمان استقرار البوت
CITIES = {
...
# 2. واجهة البداية (اختيار المدينة)
...
async def city_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = load_data()
    data[chat_id] = {"lat": CITIES[city_code]['lat'], "lon": CITIES[city_code]['lon'], "tz": CITIES[city_code]['tz'], "last_m": "", "last_e": ""}
    save_data(data)
    
    bot_info = await query.bot.get_me()
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🤖 تفعيل البوت /start", url=f"https://t.me/{bot_info.username}?start=true")],
        [InlineKeyboardButton("📢 قناة قرآن الفجر الرسمية", url=OFFICIAL_CHANNEL_LINK)]
    ])
    
    await query.edit_message_text(f"✅ تم ضبط التوقيت حسب مدينة (**{CITIES[city_code]['name']}**)\n\nستصلك الرسائل في وقتها الصحيح بإذن الله.", reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN)

# 3. البث التلقائي (أذكار الصباح والمساء)
    ...
    for chat_id, info in data.items():
        try:
            ...
            if now >= (fajr + timedelta(minutes=20)) and info.get('last_m') != str(now.date()):
                await context.bot.send_message(chat_id=int(chat_id), text="☀️ أصبحنا وأصبح الملك لله.\n📖 ﴿إِنَّ مَعَ العُسرِ يُسرًا﴾", parse_mode=ParseMode.MARKDOWN)
                info['last_m'] = str(now.date()); updated = True
            if now >= (maghrib - timedelta(minutes=20)) and info.get('last_e') != str(now.date()):
                await context.bot.send_message(chat_id=int(chat_id), text="🌙 أمسينا وأمسى الملك لله.\n🤲 يا مقلب القلوب ثبت قلبي على دينك.", parse_mode=ParseMode.MARKDOWN)
